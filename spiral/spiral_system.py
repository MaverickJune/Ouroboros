...
from spiral.models.kv_cache_model import KVCacheModelLade, KVCacheModelSimpleWithGuess
from spiral.cache_engine.spiral_cache import CacheEngine

from spiral.log_config import get_logger
logger = get_logger(__name__)

@torch.no_grad()
def activate_spiral(prefix : torch.Tensor, approx_model : torch.nn.Module, target_model : torch.nn.Module, ngram_cache : CacheEngine = None,
    ...
    if end_pos is not None:
        logger.info(f"prefix end pos : {end_pos}")
        logger.info(f"check eos id : {prefix[:, end_pos-1]}")
        prefix = prefix[:, :end_pos]
        
    else:
        logger.info("prefix end pos : None")
        logger.info(f"check eos id : {prefix[:, -1]}")
